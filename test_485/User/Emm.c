...
***	qq����Ⱥ��262438510
**********************************************************/
// UART_HandleTypeDef huart1;     /* RS485���ƾ��(����) */
/**
  * @brief    ����ǰλ������
  * @param    addr  �������ַ
  * @retval   ��ַ + ������ + ����״̬ + У���ֽ�
...
void Emm_V5_Read_Sys_Params(uint8_t addr, SysParams_t s)
{
  ...
  switch(s)                             // ������
  {
    case S_VER  : cmd[i] = 0x1F; ++i; break; // 读取固件版本和对应的硬件版本
    case S_RL   : cmd[i] = 0x20; ++i; break; //读取相电阻和相电感
    case S_PID  : cmd[i] = 0x21; ++i; break; //读取位置环PID参数
    ...
    case S_ORG  : cmd[i] = 0x3B; ++i; break; //读取回零状态标志位
    case S_Conf : cmd[i] = 0x42; ++i; cmd[i] = 0x6C; ++i; break; //读取驱动配置参数
    case S_State: cmd[i] = 0x43; ++i; cmd[i] = 0x7A; ++i; break; //读取系统状态参数
    default: break;
  }
...
void Emm_V5_Vel_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, bool snF)
{
  ...
  // װ������
  cmd[0] =  addr;                       // ��ַ
  cmd[1] =  0xF6;                       // velocity control mode
  cmd[2] =  dir;                        // ����
...
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF)
{
  ...
  rs485_send_data((uint8_t *)cmd, 13);
}
...
void Emm_V5_Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  ...
  rs485_send_data((uint8_t *)cmd, 20);  
}
